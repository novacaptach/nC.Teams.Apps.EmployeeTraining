...
public class LnDTeamController : BaseController
{
    ...
    public async Task<IActionResult> GetAllLnDTeamMembersAsync()
    {
        try
        {
            this.RecordEvent(eventName: "Get All LnD teams' members- The HTTP GET call to get all LnD teams' members has initiated");

            var allLnDTeamMembers = await this.teamInfoHelper.GetAllLnDTeamMembersAsync();

            this.RecordEvent(eventName: "Get All LnD teams' members- The HTTP GET call to get all LnD teams' members has succeeded");
            ...
            return this.Ok(value: result.OrderBy(member => member.Name));
        }
        catch (Exception ex)
        {
            this.RecordEvent(eventName: "Get All LnD teams' members- The HTTP GET call to get all LnD teams' members has failed");
            this.logger.LogError(exception: ex, message: "Error occurred while fetching all LnD teams' members");
            throw;
        }
    }
}