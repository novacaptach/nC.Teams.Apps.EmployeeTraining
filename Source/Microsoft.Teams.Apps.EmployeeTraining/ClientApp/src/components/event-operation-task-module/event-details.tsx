...
const EventDetails: React.FunctionComponent<IEventDetailsProps> = (props) => {
  ...
  const renderEventCreatorInfo = () => {
    const name =
      props.eventCreatedByName && props.eventCreatedByName.length
        ? props.eventCreatedByName
        : localize("unknownUserName");
    return (
      <Flex
        vAlign="center"
        hAlign="start"
      ...
      >
        <Avatar
          ...
          name={name}
          className={
            props.dir === LanguageDirection.Rtl ? "rtl-left-margin-small" : ""
          }
        />
        <Layout
  ...
  const renderFooter = () => {
    ...
    return (
      <Flex className="footer" vAlign="center">
        {props.isOperationFailed ? (
          <Text
            error
            content={localize("dataResponseFailedStatus")}
            weight="semibold"
          />
        ) : null}
        <Flex.Item
          grow={props.dir === LanguageDirection.Rtl}
          push={props.dir === LanguageDirection.Ltr}
        >
          <div>
            <Button
              primary
      ...
      return (
        <Flex column vAlign="center" gap="gap.smaller">
          <Text
            ...
            truncated
            content={props.eventDetails?.categoryName}
            title={props.eventDetails?.categoryName}
            weight="bold"
          />
          <Flex vAlign="center">{getArtifacts()}</Flex>
        </Flex>
      );
    } else {
      return (
    ...
    if (props.eventDetails?.type === EventType.LiveEvent) {
      return (
        ...
        >
          ...
          <a
            href={props.eventDetails?.meetingLink}
            target="_blank"
            rel="noopener noreferrer"
          >
            {props.eventDetails?.meetingLink}
          </a>
        </Flex>
      );
    }
        ...
        <Provider>
          <Flex>
            <div
              className={`${
                props.isMobileView
                  ? "mobile-task-module-container"
                  : "task-module-container"
              } event-task-module`}
            >
              <div className="event-info">
                {renderHeader()}
                {props.eventDetails.photo && (
                  <EventImage
                    className="event-image"
                    imageSrc={props.eventDetails.photo}
                  />
                )}
                {props.eventDetails.selectedColor && (
                  <div
                    className="event-image"
                    style={{
                      backgroundColor: props.eventDetails.selectedColor,
                    }}
                  >
                    <Flex
                      className="event-image"
                    ...
                    >
                      <Text
                        ...
                        size="large"
                        weight="semibold"
                        content={props.eventDetails.name}
                ...
                <div style={{ marginTop: "1.33rem" }}>
                  <Text
                    align={
                      props.dir === LanguageDirection.Rtl ? "end" : "start"
                    }
                    className={
                      props.dir === LanguageDirection.Rtl
                        ? "rtl-right-margin-smaller"
                        : ""
                    }
                    content={props.eventDetails?.name}
                    weight="bold"
                ...
                <Flex vAlign="center" hAlign="start">
                  <Layout
                    ...
                    main={
                      <Text
                        content={formatEventDayAndTimeToShort(
                ...
                >
                  <Text
                    ...
                    content={props.eventDetails?.description}
                  />
                </Flex>
                {renderAttendeeURL()}
                <Flex
                ...
                >
                  <Flex
                    ...
                    className={
                      props.dir === LanguageDirection.Rtl
                        ? "rtl-left-margin-large"
                        : ""
                    }
                  >
                    <Text
                      content={localize("totalNoOfParticipants")}
                      weight="semibold"
                    />
                    <Text
                      align={
                      ...
                      content={props.eventDetails?.maximumNumberOfParticipants}
                    />
                  </Flex>
                  <Flex column>
                    <Text
                      content={localize("registeredParticipants")}
                      weight="semibold"
                    />
                    <Text
                      align={
                      ...
                      content={props.eventDetails?.registeredAttendeesCount}
                    />
                  </Flex>
                </Flex>
