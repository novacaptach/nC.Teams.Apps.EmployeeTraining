...
class MobileFilterBar extends React.Component<
  IMobileFilterBarProps,
  IMobileFilterBarState
> {
  localize: TFunction;
  ...
  constructor(props) {
    ...
    this.sortByList = getLocalizedSortBy(this.localize).map(
      (sortBy: IPostType) => {
        return {
          key: sortBy.id,
          label: sortBy.name,
  ...
  loadEventCategories = async () => {
    const response = await getEventCategoriesAsync();
        ...
        this.setState({
          categoryList: response.data.map((value: ICategory) => {
            return {
              ...
              title: value.name,
              checkboxLabel: <Text content={value.name} />,
            };
          }),
        });
      }
  ...
  loadAllLnDTeamMembers = async () => {
    const response = await getAllLnDTeamMembersAsync();
  ...
  loadFilterItems = async () => {
    const eventCategories = this.loadEventCategories();
    const lndTeamMembers = this.loadAllLnDTeamMembers();

    Promise.all([eventCategories, lndTeamMembers]).finally(() => {
      const filteredCategories = localStorage.getItem(
        Resources.userEventsMobileFilteredCategoriesLocalStorageKey
      );
      const filteredUsers = localStorage.getItem(
        Resources.userEventsMobileFilteredUsersLocalStorageKey
      );
      const sortBy = localStorage.getItem(
        Resources.userEventsMobileSortByFilterLocalStorageKey
      );
      ...
      if (filteredCategories?.trim().length && categoryList?.length > 0) {
        const categories = categoryList.filter(
          (category: ICheckBoxItem) =>
            filteredCategories && filteredCategories.indexOf(category.key) > -1
        );
      ...
      const createdByList = [...this.state.createdByList];
      let createdBySelectedCount = 0;

      if (filteredUsers?.trim().length && createdByList?.length > 0) {
        const users = createdByList.filter(
          (user: ICheckBoxItem) =>
            filteredUsers && filteredUsers.indexOf(user.key) > -1
        );
      ...
      if (sortBy?.trim().length) {
        const sortByValue = this.sortByList?.find(
          (sortByItem: IRadioGroupItem) =>
            sortByItem.key.toString() === sortBy?.trim()
        );
  ...
  onCategoryStateChange = (categoryValues: Array<ICheckBoxItem>) => {
    const selectedCategories = categoryValues?.filter(
      (value: ICheckBoxItem) => {
  ...
  onCreatedByStateChange = (createdByValues: Array<ICheckBoxItem>) => {
    const selectedUsers = createdByValues?.filter((value: ICheckBoxItem) => {
      return value.isChecked;
    });
  ...
  onCloseIconClick = () => {
    ...
    if (
      this.state.categoryList.filter((sharedBy: ICheckBoxItem) => {
        return sharedBy.isChecked;
      }).length
    ) {
      const updatedList = this.state.categoryList.map(
        (sharedBy: ICheckBoxItem) => {
          sharedBy.isChecked = false;
          return sharedBy;
        }
      );
    ...
    if (
      this.state.createdByList.filter((tag: ICheckBoxItem) => {
        return tag.isChecked;
      }).length
    ) {
      const updatedList = this.state.createdByList.map((tag: ICheckBoxItem) => {
        tag.isChecked = false;
        return tag;
      });
  ...
  onApplyFilter = () => {
    ...
    const selectedCategories = categories.filter((category: ICheckBoxItem) => {
    ...
    const users = this.state.createdByList ? [...this.state.createdByList] : [];

    const selectedUsers = users.filter((user: ICheckBoxItem) => {
      return user.isChecked;
    });

    const selectedUserIds = selectedUsers.map((user: ICheckBoxItem) => {
      return user.key;
    });
    localStorage.setItem(
      Resources.userEventsMobileFilteredUsersLocalStorageKey,
    ...
    localStorage.setItem(
      Resources.userEventsMobileSortByFilterLocalStorageKey,
      this.state.selectedSortBy.toString()
    );

    this.props.microsoftTeams.tasks.submitTask({
      isFilterStateChanged: this.isFilterStateChanged,
    });
  };
        ...
        <Flex className="mobile-filter-buttons">
          <Flex.Item push>
            <Flex gap="gap.small" vAlign="center">
              <Button
                content={this.localize("mobileFilterClearButtonText")}
                onClick={this.onClearFilter}
              />
              <Button
                primary
                content={this.localize("mobileFilterApplyButtonText")}
        ...
        <Flex className="filter-bar mobile-filter-wrapper">
          <div className="menu-wrapper">
            <AccordionMenuWrapper
              isFilterOpen={this.state.isCategoriesFilterOpen}
              title={`${this.localize("category")} ${
                this.state.categorySelectedCount > 0
                  ? `(${this.state.categorySelectedCount} ${this.localize(
                      "selected"
                    )})`
                  : ""
              }`}
              ...
              selectedCount={this.state.categorySelectedCount}
              checkboxes={this.state.categoryList}
              onRadiogroupStateChange={this.onSortByStateChange}
              onCheckboxStateChange={this.onCategoryStateChange}
              onFilterOpenChange={this.onCategoriesFilterOpenStateChange}
            />
            <AccordionMenuWrapper
              isFilterOpen={this.state.isCreatedByFilterOpen}
              title={`${this.localize("createdBy")} ${
                this.state.createdBySelectedCount > 0
                  ? `(${this.state.createdBySelectedCount} ${this.localize(
                      "selected"
                    )})`
                  : ""
              }`}
              showSearchBar={true}
              selectedSortBy={this.state.selectedSortBy!}
              selectedCount={this.state.createdBySelectedCount}
              checkboxes={this.state.createdByList}
              onRadiogroupStateChange={this.onSortByStateChange}
              onCheckboxStateChange={this.onCreatedByStateChange}
              onFilterOpenChange={this.onCreatedByFilterOpenStateChange}
            />
            <AccordionMenuWrapper
              isFilterOpen={this.state.isSortByFilterOpen}
              title={this.sortByList[this.state.selectedSortBy!].name}
              selectedCount={this.state.createdBySelectedCount}
              selectedSortBy={this.state.selectedSortBy!}
              radioGroup={this.sortByList}
              onRadiogroupStateChange={this.onSortByStateChange}
              onCheckboxStateChange={this.onCreatedByStateChange}
              onFilterOpenChange={this.onSortByFilterOpenStateChange}
            />
          </div>
        </Flex>
      </Provider>
