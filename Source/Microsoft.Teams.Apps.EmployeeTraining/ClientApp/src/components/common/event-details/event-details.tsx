...
const EventDetails: React.FunctionComponent<IEventDetailsProps> = (props) => {
  ...
  const [eventDetails, setEventDetails] = React.useState(props.eventDetails);
  const [isLoading, setLoading] = React.useState(false);
  const [errorGettingEventDetails, setEventDetailsError] = React.useState(
    false
  );
  ...
  const getUserProfile = async (userId: string) => {
    const user: Array<string> = [userId];
    const response = await getUserProfiles(user);
  ...
  const getEventAudience = () => {
    if (eventDetails.audience === EventAudience.Private) {
      return (
        <Label
          className="category-type-label"
          circular
          icon={<EyeSlashIcon />}
          iconPosition="start"
          content={localize("private")}
        />
      );
    } else {
      return (
  ...
  const renderEventCreatorInfo = () => {
    const name =
      createdByName && createdByName.length
        ? createdByName
  ...
  const renderEventDetails = () => {
    if (isLoading) {
      return <Loader className="task-module-loader" />;
    }
        ...
        <div className="close-cancel-event">
          <Flex space="between" vAlign="center">
            <Text
              ...
              content={eventDetails.categoryName}
              weight="bold"
            />
            <Flex.Item push>{getEventAudience()}</Flex.Item>
          </Flex>
