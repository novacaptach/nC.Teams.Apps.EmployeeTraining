...
const MaxItemsToShowInFilter = 50;

const PopupMenuCheckboxesContent: React.FunctionComponent<IPopupMenuCheckboxesContentProps> = (
  props
) => {
  const localize = useTranslation().t;
  const [data, setCheckbox] = React.useState({
    checkboxes: props.content.checkboxes,
  });
  const [filteredCheckboxes, setFilteredCheckboxes] = React.useState(
    props.content.checkboxes
  );
  ...
  const onCheckboxValueChange = (key: string, checked: boolean) => {
    ...
    const checkboxList = data.checkboxes.map((checkbox: ICheckBoxItem) => {
      if (checkbox.key === key) {
    ...
    if (searchedString.trim().length) {
      const filteredItems = checkboxList.filter((item: ICheckBoxItem) => {
  ...
  const onSearchChange = (text: string) => {
    ...
    if (text.trim().length > 0) {
      const filteredItems = data.checkboxes.filter((item: ICheckBoxItem) => {
  ...
  const setFilterCheckboxes = (filterItems: any) => {
    if (filterItems && filterItems.length > 0) {
      ...
      const itemsToRender = items.slice(0, MaxItemsToShowInFilter);
        ...
        <div className="content-items-wrapper">
          {props.showSearchBar && (
            <div className="content-items-headerfooter">
              <Input
                icon={<SearchIcon />}
                iconPosition={props.dir === "rtl" ? "start" : "end"}
          ...
          <div className="content-items-headerfooter">
            <Flex gap="gap.small" vAlign="center" hAlign="end">
              ...
              <Button
                disabled={checkBoxClicked ? disableClear : props.disableClear}
                className={
                  props.selectedCount === 0
                    ? "clear-button "
                    : "clear-button enable-clear"
                }
                size="small"
                text
                onClick={() => deSelectAll()}
                content={
                  props.selectedCount > 0
                    ? localize("clear") + "(" + getSelectedCountString() + ")"
                    : localize("clear")
                }
              />
            </Flex>
            ...
            {filteredCheckboxes.map((checkbox: ICheckBoxItem) => {
              if (checkbox.title.trim().length) {
                return (
                  <Flex gap="gap.small">
                    <Checkbox
                      data-testid={checkbox.key + "_categoryCheckbox_item"}
                      className="checkbox-wrapper"
                      label={checkbox.checkboxLabel}
                      key={checkbox.key}
                      checked={checkbox.isChecked}
                      onChange={(key, data: any) =>
                        onCheckboxValueChange(checkbox.key, data.checked)
                      }
                    />
                  </Flex>
                );
