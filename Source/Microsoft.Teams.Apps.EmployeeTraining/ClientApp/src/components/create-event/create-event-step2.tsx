...
import DropdownSearch, {
  IDropdownItem,
} from "../common/user-search-dropdown/dropdown-search";
import {
  Text,
  Flex,
  Button,
...
class CreateEventStep2 extends React.Component<
  ICreateEventsStep2Props,
  ICreateEventsStep2State
> {
  ...
  constructor(props: any) {
    ...
    const audienceTypes = getLocalizedAudienceTypes(this.localize);
    this.state = {
      isLoading: false,
      selectedAudienceType: {
        key: this.props.eventPageState.eventDetails.audience?.toString()!,
        header: audienceTypes.find(
          (audience) =>
            audience.id === this.props.eventPageState.eventDetails.audience!
        )?.name!,
      },
      selectedUsersAndGroups:
        this.props.eventPageState.selectedUserGroups.length > 0
          ? this.props.eventPageState.selectedUserGroups
          : new Array<ISelectedUserGroup>(),
  ...
  nextBtnClick = () => {
    ...
    modifiedState.eventDetails.selectedUserOrGroupListJSON = JSON.stringify(
      this.state.selectedUsersAndGroups
    );
  ...
  onToggleChange = (memberIndex: number) => {
    const members = this.state.selectedUsersAndGroups;
  ...
  renderMembers = () => {
    if (
      this.state.selectedUsersAndGroups &&
      this.state.selectedUsersAndGroups.length > 0
    ) {
      const rows = this.state.selectedUsersAndGroups.map((member, index) => {
        return {
          key: index,
          items: [
            {
              content: (
                <>
                  <Text weight="bold" content={member.displayName} />
                  <br />
      ...
      return <Table className="selected-user-group-table" rows={rows} />;
    } else {
      return (
        <Flex gap="gap.small">
          <Flex.Item>
          ...
          <Flex.Item grow={this.props.dir === LanguageDirection.Ltr}>
            <Flex
              ...
              className={
                this.props.dir === LanguageDirection.Rtl
                  ? "rtl-right-margin-medium rtl-direction"
                  : ""
              }
            >
              <div>
                <Text
                  weight="bold"
                  content={this.localize("noUserSelectedHeaderStep2")}
                />
                <br />
                <Text
                  size="small"
                  content={this.localize("noUserSelectedContentStep2")}
                />
              </div>
            </Flex>
          </Flex.Item>
  ...
  onUserOrGroupSelection = async (selectedItem: IDropdownItem) => {
    const selectedUserOrGroup: ISelectedUserGroup = {
    ...
    const existingUsers = this.state.selectedUsersAndGroups;
    const isAlreadyExist = existingUsers.find(
      (userOrGroup) => userOrGroup.id === selectedUserOrGroup.id
    );
    if (!isAlreadyExist) {
  ...
  onAutoRegisterToggleChange = () => {
    this.setState((prevState: ICreateEventsStep2State) => ({
      eventDetails: {
        ...
        isAutoRegister: !this.state.eventDetails.isAutoRegister,
      },
    }));
  };
  ...
  onMandatoryAllClocked = () => {
    const selectedUserGroup = [...this.state.selectedUsersAndGroups];
    for (let i = 0; i < selectedUserGroup.length; i++) {
      ...
      <React.Fragment>
        <div className="page-content">
          <Flex gap="gap.smaller">
            <Text
              size="large"
              content={this.localize("audienceDetailsStep2")}
            />
          </Flex>
            ...
            <Flex.Item size="size.half">
              <Flex gap="gap.smaller" vAlign="center">
                <Text
                  className={
                    this.props.dir === LanguageDirection.Rtl
                      ? "rtl-left-margin-small form-label"
                      : "form-label"
                  }
                  content={this.localize("audienceTypeStep2")}
                />
                <InfoIcon
                  outline
                  title={this.localize("audienceTypeInfoIconTitle")}
                />
              </Flex>
            </Flex.Item>
          </Flex>
          <Flex gap="gap.smaller" className="input-label-margin-between">
            <Flex.Item size="size.half">
              <Dropdown
                ...
                items={this.state.audienceTypes.map(
                  (value: IConstantDropdownItem) => {
                    return { key: value.id, header: value.name };
                  }
                )}
                onChange={(event, data) => {
                  this.onAudienceChange(data.value);
                }}
            ...
            <>
              <Flex gap="gap.smaller" className="margin-top">
                <DropdownSearch
                  ...
                  noResultMessage={this.localize(
                    "noResultFoundDropdownMessage"
                  )}
              ...
              <Flex gap="gap.smaller" className="input-label-margin-between">
                <Checkbox
                  onChange={() => this.onAutoRegisterToggleChange()}
                  checked={this.state.eventDetails.isAutoRegister}
                  label={this.localize("autoRegisterCheckboxLabelStep2")}
                ...
                >
                  <Button
                    ...
                    disabled={
                      this.state.selectedUsersAndGroups.filter(
                        (userOrGroup) => userOrGroup.isMandatory === false
                      ).length === 0
                    }
        ...
        {this.props.dir === LanguageDirection.Rtl && (
          <Flex gap="gap.smaller" className="button-footer" vAlign="center">
            ...
            <Text
              ...
              weight="bold"
              content={this.localize("step2Of3")}
            />
            {(!this.props.eventPageState.isEdit ||
              (this.props.eventPageState.isEdit &&
                this.props.eventPageState.isDraft)) && (
