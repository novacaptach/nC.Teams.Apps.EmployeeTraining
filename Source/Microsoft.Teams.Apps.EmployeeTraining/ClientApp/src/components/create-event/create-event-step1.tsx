...
class CreateEventStep1 extends React.Component<
  ICreateEventsStep1Props,
  ICreateEventsStep1State
> {
  ...
  constructor(props: any) {
    ...
    const eventTypes = getLocalizedEventTypes(this.localize);
    ...
    this.state = {
      ...
      isLoading: false,
      categories:
        this.props.eventPageState.categories.length > 0
          ? this.props.eventPageState.categories
          : new Array<IConstantDropdownItem>(),
      theme: "",
      ...
      selectedEventType:
        this.props.eventPageState.eventDetails.type === undefined
          ? {
              key: EventType.Teams.toString(),
              header: eventTypes.find((event) => event.id === EventType.Teams)!
                .name!,
            }
          : {
              key: eventTypes
                .find(
                  (event) =>
                    event.id === this.props.eventPageState.eventDetails.type
                )!
                .id.toString(),
              header: eventTypes.find(
                (event) =>
                  event.id === this.props.eventPageState.eventDetails.type
              )!.name!,
            },
  ...
  UNSAFE_componentWillReceiveProps(nextProps: ICreateEventsStep1Props) {
    if (
      nextProps.eventPageState.categories !==
      this.props.eventPageState.categories
    ) {
    ...
    if (
      nextProps.eventPageState.eventDetails !==
      this.props.eventPageState.eventDetails
    ) {
      this.setState({
        eventDetails: { ...nextProps.eventPageState.eventDetails },
      });
    }

    if (
      nextProps.eventPageState.selectedCategory !==
      this.props.eventPageState.selectedCategory
    ) {
      this.setState({
        selectedCategory: nextProps.eventPageState.selectedCategory!,
      });
    }

    if (
      nextProps.teamsContext &&
      nextProps.teamsContext !== this.props.teamsContext
    ) {
  ...
  checkIfSaveAsDraftAllowed = () => {
    const eventValidationStatus = {
      ...
      isEventCategoryValid: true,
      isVenueValid: true,
      isMaxNoofPartipantsValid: true,
      isMeetingLinkValid: true,
    };

    if (
      this.state.eventDetails.name == "" ||
      this.state.eventDetails.name.length > Resources.eventNameMaxLength
    ) {
    ...
    this.setState({
      ...
      isMaxNoofParticipantsValid:
        eventValidationStatus.isMaxNoofPartipantsValid,
      isMeetingLinkValid: eventValidationStatus.isMeetingLinkValid,
    });

    if (
      eventValidationStatus.isPhotoValid &&
      eventValidationStatus.isColorValid &&
      eventValidationStatus.isNameValid &&
      eventValidationStatus.isDescriptionValid &&
      eventValidationStatus.isEventTypeValid &&
      eventValidationStatus.isEventCategoryValid &&
      eventValidationStatus.isVenueValid &&
      eventValidationStatus.isMaxNoofPartipantsValid &&
      eventValidationStatus.isMeetingLinkValid &&
  ...
  checkIfNextAllowed = async () => {
    ...
    if (
      !this.state.eventDetails.photo &&
      !this.state.eventDetails.selectedColor
    ) {
    ...
    if (
      this.state.eventDetails.name.trim() == "" ||
      this.state.eventDetails.name.length > Resources.eventNameMaxLength
    ) {
      eventValidationStatus.isNameValid = false;
    }

    let isEventNameAlreadyExists = false;
    ...
    if (eventValidationStatus.isNameValid) {
      ...
      if (
        !this.props.eventPageState.isEdit ||
        this.props.eventPageState.eventDetails.name !==
          this.state.eventDetails.name.trim()
      ) {
    ...
    if (
      this.state.eventDetails.description.trim() == "" ||
      this.state.eventDetails.description.length >
        Resources.eventDescriptionMaxLength
    ) {
    ...
    if (
      ...
      (this.state.eventDetails.meetingLink == "" ||
        !this.state.eventDetails.meetingLink.match(Resources.validUrlRegExp))
    ) {
    ...
    if (this.checkEventAlreadyStarted()) {
      if (
        ...
        !this.state.eventDetails!.endTime ||
        this.state.eventDetails!.startTime! >= this.state.eventDetails!.endTime!
      ) {
    ...
    if (
      ...
      eventValidationStatus.isMaxNoofPartipantsValid &&
      eventValidationStatus.isMeetingLinkValid &&
      eventValidationStatus.isTimeValid &&
      !this.state.isEventNameExisting
    ) {
      return true;
    } else {
  ...
  checkEventAlreadyStarted = () => {
    return !(
      this.props.eventPageState.isEdit &&
      moment.utc(this.state.eventDetails.startDate).local().toDate() <
        new Date()
    );
  };
  ...
  nextBtnClick = async () => {
    ...
    if (isSubmitAllowed) {
      const modifiedState = { ...this.props.eventPageState };
    ...
    if (!this.state.isNameValid) {
      return (
        <Text
          ...
          content={this.localize("required")}
          error
        />
      );
    } else if (
      ...
      return (
        <Text
          ...
          content={this.localize("eventNameMaxCharError")}
          error
        />
      );
    ...
    if (!this.state.isMeetingLinkValid) {
      return (
        <Text
          ...
          content={this.localize("validUrlErrorMessage")}
          error
        />
      );
    }
  ...
  getDescriptionError = () => {
    ...
    if (
      ...
      return (
        <Text
          data-testid="event_desc_max_error"
          content={this.localize("eventDescriptionMaxCharError")}
          error
        />
      );
    }
    ...
    if (this.state.eventDetails.venue.length > Resources.eventVenueMaxLength) {
      return (
        <Text
          ...
          content={this.localize("eventVenueMaxCharError")}
          error
        />
      );
    }
  ...
  onMaxNoOfParticipantsChange = (eventMaxNoofParticipants: string) => {
    this.setState((prevState: ICreateEventsStep1State) => ({
      eventDetails: {
        ...prevState.eventDetails,
        maximumNumberOfParticipants: parseInt(eventMaxNoofParticipants),
      },
      isMaxNoofParticipantsValid: true,
    }));
  };
  ...
  onStartTimeChange = (hours: number, min: number) => {
    this.setState((prevState: ICreateEventsStep1State) => ({
      eventDetails: {
        ...prevState.eventDetails,
        startTime: new Date(new Date().setHours(hours, min)),
        endTime:
          this.state.eventDetails.endTime === undefined
            ? new Date(new Date().setHours(hours, min + 15))
            : this.state.eventDetails.endTime,
      },
    }));
  };
  ...
  onEndTimeChange = (hours: number, min: number) => {
    this.setState((prevState: ICreateEventsStep1State) => ({
      eventDetails: {
        ...prevState.eventDetails,
        endTime: new Date(new Date().setHours(hours, min)),
      },
    }));
  };
  ...
  uploadImage = async (formData: FormData) => {
    ...
    const response = await uploadEventImage(formData, this.teamId);
    ...
    if (fileUploaded) {
      ...
      img.onload = () => {
        if (
          ...
          formData.append("fileInfo", fileUploaded, fileUploaded.name);
  ...
  checkEventName = async () => {
    const response = await searchEventAsync(this.state.eventDetails.name);
    if (response && response.status === ResponseStatus.OK && response.data) {
      const isEventAlreadyExists = response.data.some(
        (event: IEvent) =>
          event.name.toLowerCase() ===
          this.state.eventDetails.name.trim().toLowerCase()
      );
  ...
  saveEventAsDraft = async () => {
    const result = this.checkIfSaveAsDraftAllowed();
    if (result) {
      ...
      const result = await saveEventAsDraftAsync(modifiedState, this.teamId);
      if (result) {
        this.props.microsoftTeams.tasks.submitTask({
          isSuccess: true,
          isDraft: true,
        });
      } else {
  ...
  render() {
    ...
    for (var i = 0; i < this.state.categories.length; i++) {
      categoriesList.push({
        key: this.state.categories[i].id.toString(),
        header: this.state.categories[i].name,
      });
    }
      ...
      <>
        <div className="page-content">
          ...
          <Flex gap="gap.smaller" className="margin-top" vAlign="center">
            <Text
              className="form-label margin-right"
              content={this.localize("eventPhotoStep1")}
            />
            <ExclamationCircleIcon
              title={this.localize("imageInfoIcon")}
              size="small"
            />
          ...
          {this.state.eventDetails.photo && (
            <Flex
              gap="gap.smaller"
              className="input-label-margin-between"
              vAlign="end"
            >
              <FluentImage
                className="event-image-color"
                fluid
                src={this.state.eventDetails.photo}
              />
            </Flex>
          )}
          {this.state.eventDetails.selectedColor && (
            ...
            >
              <div
                className="event-image-color"
                style={{
                  backgroundColor: this.state.eventDetails.selectedColor,
                }}
              >
                <Flex
                  className="event-image-color"
                ...
                >
                  <Text
                    className="event-color-text"
                    size="large"
                    weight="semibold"
                    content={this.state.eventDetails.name}
                  />
                </Flex>
              </div>
            </Flex>
          ...
          >
            <Flex.Item>
              <>
                <Button
                  disabled={this.state.imageUploadLoader}
                  loading={this.state.imageUploadLoader}
                  onClick={this.handleUploadClick}
                  size="medium"
                  icon={
                    <FilesUploadIcon
                      className={
                        this.props.dir === LanguageDirection.Rtl
                          ? "rtl-left-margin-small"
                          : ""
                      }
                    />
                  }
                  content={
                    this.state.eventDetails.photo &&
                    this.state.eventDetails.photo.length
                      ? this.localize("changePhotoStep1")
                      : this.localize("uploadPhotoStep1")
                  }
                  iconPosition="before"
                />
                <input
                ...
                <Text
                  className={
                    this.props.dir === LanguageDirection.Rtl
                      ? "form-label rtl-left-margin-small"
                      : "form-label"
                  }
                  content={this.localize("orLabelForImageOrColor")}
                />
              </>
            </Flex.Item>
            <Flex.Item>
              <div>
                <ColorPicker
                  ...
                  selectedColor={this.state.eventDetails.selectedColor}
                />
              </div>
            </Flex.Item>
          </Flex>
          <Flex className="margin-top" gap="gap.smaller">
            <Text
              ...
              content={this.localize("eventNameStep1")}
            />
            <Flex.Item push grow={this.props.dir === LanguageDirection.Rtl}>
              {this.getNameError()}
            </Flex.Item>
          ...
          <Flex gap="gap.smaller" className="input-label-margin-between">
            <Input
              ...
              maxLength={Resources.eventNameMaxLength}
              fluid
              placeholder={this.localize("eventNamePlaceholderStep1")}
              value={this.state.eventDetails.name}
              onChange={(event: any) =>
                this.onEventNameChange(event.target.value)
              }
            />
          </Flex>
          <Flex gap="gap.smaller" className="margin-top">
            <Flex.Item size="size.half">
              <Flex>
                <Text
                  className="form-label"
                  content={this.localize("eventTypeStep1")}
                />
                <Flex.Item push grow={this.props.dir === LanguageDirection.Rtl}>
          ...
          <Flex gap="gap.smaller" className="input-label-margin-between">
            <Flex.Item size="size.half">
              <Dropdown
                className={
                  this.props.dir === LanguageDirection.Rtl
                    ? "dropdown-flex-half rtl-left-margin-small"
                    : "dropdown-flex-half"
                }
                fluid
                items={this.state.eventTypes.map(
                  (value: IConstantDropdownItem) => {
            ...
            <Flex.Item size="size.half">
              <Dropdown
                ...
                fluid
                onChange={(event, data) => {
                  onEventCategoryChange(data.value);
                }}
          ...
          {this.state.eventDetails.type === EventType.LiveEvent && (
            <>
              <Flex className="margin-top" gap="gap.smaller">
                <Text
                  className="form-label"
                  content={this.localize("liveEventUrlStep1")}
                />
                <Flex.Item push grow={this.props.dir === LanguageDirection.Rtl}>
              ...
              <Flex gap="gap.smaller" className="input-label-margin-between">
                <Input
                  fluid
                  value={this.state.eventDetails.meetingLink}
                  placeholder={this.localize("liveEventUrlPlaceholder")}
                  onChange={(event: any) =>
                    this.onLiveEventUrlChange(event.target.value)
                  }
            ...
            <>
              ...
              <Flex gap="gap.smaller" className="input-label-margin-between">
                <Input
                  ...
                  onChange={(event: any) =>
                    this.onEventVenueChange(event.target.value)
                  }
                  fluid
          ...
          <Flex gap="gap.smaller" className="margin-top">
            <Text
              className="form-label"
              content={this.localize("eventDescriptionStep1")}
            />
            <Flex.Item push grow={this.props.dir === LanguageDirection.Rtl}>
          ...
          <Flex gap="gap.smaller" className="input-label-margin-between">
            <TextArea
              ...
              onChange={(event: any) =>
                this.onEventDescriptionChange(event.target.value)
              }
          ...
          <Flex gap="gap.smaller" className="margin-top">
            <Text
              ...
              truncated
              className="form-label"
              content={`${this.localize("yourTimeZone")}: ${this.timeZone}`}
            />
          </Flex>
                ...
                <Flex.Item size="size.half">
                  <div>
                    <Text
                      ...
                      content={`${this.localize("startDateStep1")}`}
                    />
                    <Flex
                      className={
                        this.props.dir === LanguageDirection.Rtl
                    ...
                    >
                      <StartDate
                        ...
                        disableSelectionForPastDate={
                          !this.checkEventAlreadyStarted()
                        }
                      />
                    </Flex>
                  </div>
                ...
                <Flex.Item size="size.half">
                  <div>
                    <Text
                      ...
                      content={`${this.localize("endDateStep1")}`}
                    />
                    <StartDate
                      screenWidth={this.state.screenWidth}
                      theme={this.state.theme}
              ...
              <Flex gap="gap.smaller">
                <Flex.Item size="size.half">
                  <Text
                    className="form-label"
                    align={
                      this.props.dir === LanguageDirection.Rtl ? "end" : "start"
                    }
                    content={this.localize("startTimeStep1")}
                  />
                </Flex.Item>
                <Flex.Item size="size.half">
                  <Text
                    ...
                    content={this.localize("endTimeStep1")}
                  />
                </Flex.Item>
              </Flex>
            </Flex.Item>
            <Flex.Item size="size.half">
              <Flex>
                <Text
                  ...
                  content={this.localize("maxParticipantsStep1")}
                />
                <Flex.Item
                  push={this.props.dir === LanguageDirection.Ltr}
                  grow={this.props.dir === LanguageDirection.Rtl}
                >
          ...
          <Flex gap="gap.smaller" className="input-label-margin-between">
            ...
            >
              <Input
                ...
                min={1}
                fluid
                value={this.state.eventDetails.maximumNumberOfParticipants.toString()}
                onChange={(event: any) =>
                  this.onMaxNoOfParticipantsChange(event.target.value)
                }
        ...
        {this.props.dir === LanguageDirection.Ltr && (
          <Flex gap="gap.smaller" className="button-footer" vAlign="center">
            {!this.state.isTimeValid && (
              <Text error content={this.localize("startAndEndTimeError")} />
            )}
            <Flex.Item push>
            ...
            {(!this.props.eventPageState.isEdit ||
              (this.props.eventPageState.isEdit &&
                this.props.eventPageState.isDraft)) && (
              <Button
                disabled={this.state.isLoading || this.state.isValidatingStep1}
                loading={this.state.isLoading}
                onClick={this.saveEventAsDraft}
            ...
            <Button
              ...
              content={this.localize("nextButton")}
              loading={this.state.isValidatingStep1}
              disabled={this.state.isLoading || this.state.isValidatingStep1}
              primary
          ...
          <Flex gap="gap.smaller" className="button-footer" vAlign="center">
            <Flex.Item push>
              <Text
                ...
                content={
                  !this.state.isTimeValid
                    ? this.localize("startAndEndTimeError")
                    : ""
                }
              />
            </Flex.Item>
            <Text
              className="rtl-left-margin-small"
              weight="bold"
              content={this.localize("step1Of3")}
            />
            {(!this.props.eventPageState.isEdit ||
              (this.props.eventPageState.isEdit &&
                this.props.eventPageState.isDraft)) && (
