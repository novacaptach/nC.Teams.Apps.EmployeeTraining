...
class CreateEventStep3 extends React.Component<
  ICreateEventsStep3Props,
  ICreateEventsStep3State
> {
  ...
  componentDidMount() {
    ...
    if (
      this.props.eventPageState.eventDetails.audience ===
        EventAudience.Private &&
      this.props.eventPageState.eventDetails.isAutoRegister
    ) {
  ...
  getRegisteredAttendeesCount = async () => {
    const result = await validateSelectedUsers([
      ...this.props.eventPageState.selectedUserGroups,
    ]);
  ...
  /** Renders artifacts */
  getArtifacts = () => {
    const artifactElements: Array<JSX.Element> = [];
    ...
    switch (this.props.eventPageState.eventDetails?.audience) {
      case EventAudience.Private:
        artifactElements.push(
          <AudienceArtifact audienceType={EventAudience.Private} />
        );
        break;

      case EventAudience.Public:
        artifactElements.push(
          <AudienceArtifact audienceType={EventAudience.Public} />
        );
        break;
  ...
  getEventDayAndTime = (startDate: Date, startTime: Date, endTime: Date) => {
    const eventDay = moment.utc(startDate).local().format("ddd");
    const eventStartTime = moment.utc(startTime).local().format("HH:mm");
    const eventEndTime = moment.utc(endTime).local().format("HH:mm");
    ...
    if (eventType === EventType.InPerson) {
      return (
        <React.Fragment>
          <Divider
            vertical
            styles={{ height: "1.13rem", marginRight: ".8rem" }}
          />
          <Text
  ...
  saveEventAsDraft = async () => {
    ...
    const result = await saveEventAsDraftAsync(
      { ...this.props.eventPageState },
      this.teamId
    );
    if (result) {
  ...
  createEvent = async () => {
    ...
    const result = await createEvent(this.props.eventPageState, this.teamId);
    if (result) {
    ...
    if (this.props.eventPageState.eventDetails.type === EventType.LiveEvent) {
      return (
        ...
        >
          ...
          />
          <a
            href={this.props.eventPageState.eventDetails.meetingLink}
            target="_blank"
            rel="noopener noreferrer"
          >
            {this.props.eventPageState.eventDetails.meetingLink}
          </a>
        </Flex>
      );
    }
        ...
        <div className="page-content create-event-step3">
          ...
          <Flex className="margin-top" space="between" vAlign="center">
            <Text
              ...
              content={this.props.eventPageState.selectedCategory?.header!}
              weight="bold"
            />
          ...
          {this.props.eventPageState.eventDetails.photo && (
            <Image
              className="event-image"
              fluid
              src={this.props.eventPageState.eventDetails.photo}
            />
          )}
          {this.props.eventPageState.eventDetails.selectedColor && (
            <div
              className="event-image"
              style={{
                backgroundColor: this.props.eventPageState.eventDetails
                  .selectedColor,
              }}
            >
              <Flex
              ...
              >
                <Text
                  ...
                  size="large"
                  weight="semibold"
                  content={this.props.eventPageState.eventDetails.name}
                />
              </Flex>
            </div>
          )}
          <div style={{ marginTop: "1.33rem" }}>
            <Text
              align={this.props.dir === LanguageDirection.Rtl ? "end" : "start"}
              content={this.props.eventPageState.eventDetails.name}
              weight="bold"
              size="medium"
            />
          ...
          <Flex vAlign="center" hAlign="start">
            <Layout
              ...
              main={
                <Text
                  content={this.getEventDayAndTime(
                    this.props.eventPageState.eventDetails.startDate,
                    this.props.eventPageState.eventDetails.startTime!,
                    this.props.eventPageState.eventDetails.endTime!
                  )}
                  weight="semibold"
                  size="small"
                />
              ...
              gap=".4rem"
            />
            {this.renderEventVenue(
              this.props.eventPageState.eventDetails.type,
              this.props.eventPageState.eventDetails.venue
            )}
          </Flex>
          <Flex
            vAlign="center"
          ...
          >
            <Text
              align={this.props.dir === LanguageDirection.Rtl ? "end" : "start"}
              content={this.props.eventPageState.eventDetails.description}
            />
          </Flex>
          {this.renderAttendeeURL()}
          <Flex
            gap={
              this.props.dir === LanguageDirection.Rtl ? undefined : "gap.large"
            }
            design={{ marginTop: "2.67rem" }}
          >
            <Flex
            ...
            >
              <Text
                ...
                content={this.localize("totalNoOfParticipants")}
                weight="semibold"
              />
              <Text
                align={
            ...
            <Flex column>
              <Text
                ...
                content={this.localize("registeredParticipants")}
                weight="semibold"
              />
              <Text
                align={
                ...
                content={this.state.registeredAttendeesCount}
              />
            </Flex>
          </Flex>
        </div>
          ...
          <Flex gap="gap.smaller" className="button-footer" vAlign="center">
            ...
            {(this.props.eventPageState.isDraft ||
              ...
              <>
                <Button
                  disabled={
                    this.state.isCreateLoading || this.state.isDraftLoading
                  }
                  loading={this.state.isDraftLoading}
              ...
              this.props.eventPageState.isEdit && (
                <>
                  <Button
                    disabled={this.state.isCreateLoading}
                    loading={this.state.isCreateLoading}
                    content={this.localize("updateEventButton")}
                    primary
        ...
        {this.props.dir === LanguageDirection.Rtl && (
          <Flex gap="gap.smaller" className="button-footer" vAlign="center">
            ...
            <Text
              ...
              weight="bold"
              content={this.localize("step3of3")}
            />
            {(this.props.eventPageState.isDraft ||
              !this.props.eventPageState.isEdit) && (
              <>
