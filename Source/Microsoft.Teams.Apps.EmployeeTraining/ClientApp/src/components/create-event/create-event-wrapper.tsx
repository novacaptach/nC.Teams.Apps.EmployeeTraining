...
class CreateEventWrapper extends React.Component<
  ICreateEventProps,
  ICreateEventState
> {
  readonly localize: TFunction;
  minDate = new Date();
  params: { eventId?: string | undefined; isDraft?: boolean | undefined } = {
    eventId: undefined,
    isDraft: false,
  };
  teamId: string;
        ...
        this.setState(
          {
            isLoading: false,
            eventDetails: eventDetails,
            selectedUserGroups: selectedUsersOrGroups,
            isEdit: true,
            isDraft: this.params.isDraft!,
          },
          this.getAllCategories
        );
      }
  ...
  getAllCategories = async () => {
    ...
    if (response.status === ResponseStatus.OK) {
      const categories: any = response.data.map((category: any) => {
        return { name: category.name, id: category.categoryId };
      });

      if (this.state.eventDetails.categoryId) {
        const category = categories.find(
          (category: any) => category.id === this.state.eventDetails.categoryId
        );
  ...
  setEventStep = (currentStep: number, stepEventState: ICreateEventState) => {
    this.setState({
      currentEventStep: currentStep,
      eventDetails: stepEventState.eventDetails,
      selectedAudience: stepEventState.selectedAudience,
      selectedCategory: stepEventState.selectedCategory,
      selectedEvent: stepEventState.selectedEvent,
      selectedUserGroups: stepEventState.selectedUserGroups,
      categories: stepEventState.categories,
    });
  };
      ...
      case 1:
        return (
          <CreateEventStep1
            ...
            navigateToPage={this.setEventStep}
          />
        );
      case 2:
        return (
        ...
        <Provider>
          <Flex>
            <div className="task-module-container event-task-module">
              <Text
                error
                content={this.localize("categoriesNotAvailableError")}
                weight="semibold"
              />
            </div>
          </Flex>
