...
interface IListCategoriesProps extends WithTranslation, IWithContext {
  ...
  onActionPerformed: (
    categoryOperation: CategoryOperations,
...
> {
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      isLoading: false,
      showStatusIcon:
        this.props.statusMessage !== "" && this.props.statusMessage.length > 0,
    };
  }
  ...
  getCategoriesAsync = async (teamId: string) => {
    const response = await getCategoriesAsync(teamId);
    ...
    if (response && response.status === ResponseStatus.OK) {
      ...
      this.setState({
        isLoading: false,
        statusMessage: this.localize("dataResponseFailedStatus"),
      });
    }
  };
  ...
  manageControlsEnabilityAndSelection() {
    const selectedCategoriesCount = this.state.categories.filter(
      (category: ICategory) => {
    ...
    const isAllCategoriesSelected =
      selectedCategoriesCount === this.state.categories.length;
    ...
    if (selectedCategoriesCount === 1) {
      statusMessage = this.localize("listCategoriesSelectedStatus", {
        ...
        totalCategoriesCount: this.state.categories.length,
      });
      this.setState({
        isEditEnabled: true,
  ...
  getLastCategoryUpdateDateAndTime = (): any => {
    if (this.state.categories?.length > 0) {
      ...
      this.localize("lastUpdatedOnStatus", {
        lastUpdatedOnStatus:
          categories[categories.length - 1].updatedOn?.toUTCString,
      });
    } else {
      return null;
    }
  ...
  searchCategories = (searchText: string) => {
    const categories: Array<ICategory> = this.state.categories ?? [];
    ...
    if (categories.length > 0 && searchText && searchText.trim() !== "") {
      ...
      const searchedCategories = categories.filter((category) => {
        return (
          category.name.toLowerCase().indexOf(this.searchText.toLowerCase()) >
            -1 ||
  ...
  onActionPerformed = (categoryOperation: CategoryOperations) => {
    if (
      categoryOperation === CategoryOperations.Edit ||
      categoryOperation === CategoryOperations.Delete
    ) {
      const categoryOperationData = this.state.categories.filter(
      ...
      return (
        <React.Fragment>
          ...
          <Flex.Item push>
            <Text
              timestamp
              content={this.getLastCategoryUpdateDateAndTime()}
              weight="semibold"
            />
          </Flex.Item>
        </React.Fragment>
  ...
  renderCategories = () => {
    ...
    if (categories?.length > 0) {
        ...
        items: [
          {
            ...
            content: (
              <Checkbox
                ...
                key="categoriesTableHeader"
                checked={this.state.isAllCategoriesSelected}
                onChange={this.onSelectAllCategoriesCheckedChange}
              />
            ),
          },
          {
            ...
            content: this.localize("categoryName"),
          },
          {
      ...
      const rows = categories.map((category: ICategory, index: number) => {
        return {
          key: index,
          items: [
            {
              ...
              content: (
                <Checkbox
                  data-testid={`list-categories_categoryCheckbox${index.toString()}`}
                  key={index}
                  checked={category.isSelected}
                  onChange={() => this.onCategoryCheckedChange(category)}
                />
              ),
            },
            {
              ...
              <div
                className={
                  this.props.dir === LanguageDirection.Rtl
                    ? "rtl-direction rtl-right-margin-small"
                    : ""
                }
              >
                <Text
                  weight="bold"
                  content={this.localize("categoriesNotAvailableHeader")}
                />
                <br />
                <Text
                  content={
                    this.searchText !== ""
                      ? this.localize(
                          "categoriesNotFoundForSearchedTextDescription",
        ...
        <div className="commandbar-wrapper">
          <Flex className="commandbar-wrapper-container">
            <Button
              ...
              className="list-categories-menu-button"
              icon={
                <AddIcon
                  className={
                    this.props.dir === LanguageDirection.Rtl
              ...
              content={this.localize("add")}
              onClick={() => this.onActionPerformed(CategoryOperations.Add)}
            />
            <Button
              text
              ...
              onClick={() => this.onActionPerformed(CategoryOperations.Edit)}
            />
            <Button
              text
              ...
              disabled={!this.state.isDeleteEnabled}
              content={this.localize("delete")}
              onClick={() => this.onActionPerformed(CategoryOperations.Delete)}
            />
            <Flex.Item
