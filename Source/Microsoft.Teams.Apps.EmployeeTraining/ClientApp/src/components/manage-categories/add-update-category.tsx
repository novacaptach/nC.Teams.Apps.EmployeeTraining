...
class AddUpdateCategory extends React.Component<
  IAddCategoryProps,
  IAddCategoryState
> {
  ...
  createOrUpdateCategoryAsync = async () => {
    ...
    if (
      !this.props.category ||
      this.props.category.name !== this.state.category.name.trim()
    ) {
    ...
    if (this.props.category) {
      ...
      if (response.status == ResponseStatus.OK) {
        if (response.data == true) {
          this.props.onCategoryAddedOrUpdated(
            this.localize("categoryUpdateSuccess")
          );
        } else {
          this.setState({
            isCreatingOrUpdating: false,
            errorMessage: this.localize("failedToUpdateCategory"),
          });
        }
      } else {
        this.setState({
          isCreatingOrUpdating: false,
          errorMessage: this.localize("dataResponseFailedStatus"),
        });
      }
    } else {
      ...
      if (response.status == ResponseStatus.OK && response.data === true) {
        ...
        this.props.onCategoryAddedOrUpdated(
          this.localize("categoryAddedSuccess")
        );
      } else {
        this.setState({
          isCreatingOrUpdating: false,
  ...
  checkCategoryExists = async () => {
    if (this.state.category.name?.length) {
      const result = await getEventCategoriesAsync();

      if (result && result.status === ResponseStatus.OK && result.data) {
        const categories: Array<ICategory> = result.data;

        if (!categories || categories.length === 0) {
          return false;
        }

        const isCategoryAlreadyExists = categories.some(
          (category: ICategory) =>
            category.name.toLowerCase() ===
            this.state.category.name.trim().toLowerCase()
        );
    ...
    if (this.state.isCategoryAlreadyExists) {
      return (
        <Flex.Item push>
          <Text
            error
            content={this.localize("categoryAlreadyExistsError")}
            weight="semibold"
          />
        </Flex.Item>
      );
        ...
        <Flex column gap="gap.medium" data-testid="addorupdateform">
          ...
          <Flex column gap="gap.smaller">
            ...
            <TextArea
              ...
              fluid
              placeholder={this.localize("categoryDescriptionPlaceholder", {
                categoryDescriptionMaxLength:
                  Constants.categoryDescriptionMaxLength,
              })}
          ...
          <ManageCategoriesOperationFooter
            ...
            submitButtonContent={
              this.props.category
                ? this.localize("save")
                : this.localize("addCategory")
            }
            isSubmitButtonDisabled={
              this.state.category.name.trim() === "" ||
              this.state.category.description.trim() === "" ||
              this.state.isCreatingOrUpdating ||
              this.state.isCategoryAlreadyExists
            }
