...
> {
  ...
  deleteCategoryAsync = async () => {
    ...
    const categoryIdsToDelete = this.props.categories.map((category) => {
      return category.categoryId;
    });

    const response = await deleteCategoriesAsync(
      this.teamId,
      categoryIdsToDelete.join(",")
    );
    ...
    if (response.status === ResponseStatus.OK) {
      if (response.data === true) {
        ...
        this.setState({
          isDeleting: false,
          errorMessage: this.localize("failedToDeleteCategory"),
        });
      }
    } else {
      this.setState({
  ...
  renderCategories = () => {
    if (this.props.categories && this.props.categories.length > 0) {
      ...
      const rows = this.props.categories.map(
            ...
            items: [
              {
                ...
                content: category.isInUse ? (
                  <ExclamationCircleIcon
                    title={this.localize("categoryInUse")}
                    style={{ paddingLeft: ".42rem" }}
                  />
                ) : (
                  ""
                ),
  ...
  render() {
    return (
      <React.Fragment>
        <Text
          content={this.localize("deleteCategoryConfirmation")}
          weight="semibold"
        />
