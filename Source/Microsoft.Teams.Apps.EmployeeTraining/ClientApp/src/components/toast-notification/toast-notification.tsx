...
> {
  ...
  componentWillReceiveProps(nextProps: IToastNotificationProps) {
    if (
      nextProps.notification.id !== this.props.notification.id &&
      nextProps.notification.message?.length &&
      nextProps.notification.type !== ActivityStatus.None
    ) {
    ...
    return (
      <Toast
        className={`notification-toast ${
          this.props.notification.type === ActivityStatus.Success
            ? "success"
            : "error"
        }`}
        autohide
