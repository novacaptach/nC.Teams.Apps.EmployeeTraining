...
> {
  ...
  componentDidMount() {
    ...
    const mandatoryEvents = this.getEventsAsync(
      EventSearchType.MandatoryEventsForUser
    );
    const allEvents = this.getEventsAsync(
  ...
  loadEvents = async (
    ...
    const events = await this.getEventsAsync(eventSearchType, pageNumber);
      ...
      switch (eventSearchType) {
        case EventSearchType.MandatoryEventsForUser:
          if (pageNumber > 0) {
            ...
            const mandatoryEvents = [...this.state.mandatoryEvents];

            const updatedEventResponse = response.filter(
              (event: IEvent) =>
                mandatoryEvents.findIndex(
                  (eventDetails: IEvent) =>
  ...
  onEventClick = (eventDetails: IEvent) => {
    this.props.microsoftTeams.tasks.startTask(
      ...
      (error: any, result: any) => {
        if (result) {
          const events =
            this.state.activeTabIndex === MandatoryEventsTabIndex
              ? [...this.state.mandatoryEvents]
              : [...this.state.allEvents];
          const eventAtIndex = events?.findIndex(
            (event: IEvent) => event.eventId === eventDetails.eventId
          );

          const eventToUpdate: IEvent = events[eventAtIndex];
          eventToUpdate.registeredAttendeesCount =
            result.type === EventOperationType.Register
              ? eventToUpdate.registeredAttendeesCount + 1
              : eventToUpdate.registeredAttendeesCount - 1;
          ...
          if (this.state.activeTabIndex === MandatoryEventsTabIndex) {
            this.setState((prevState: IDiscoverEventsState) => ({
              ...
              notification: {
                ...
                message:
                  result.type === EventOperationType.Register
                    ? this.localize("eventRegistrationSuccessfulMessage")
                    : this.localize("eventUnregistrationSuccessfulMessage"),
  ...
  onTabIndexChange = (event: MenuProps | undefined) => {
    ...
    this.setState(
      {
        isLoadingEvents: true,
        activeTabIndex: event?.activeIndex!,
        searchText: "",
        filteredCategories: "",
        filteredUsers: "",
        sortByFilter: SortBy.Recent,
  ...
  onFilterChange = (
    ...
    const filteredCategories = selectedCategories ? selectedCategories : [];
    const filteredUsers = selectedUsers ? selectedUsers : [];
  ...
  getEvents = (pageNumber: number = 0) => {
    if (pageNumber === 0) {
      this.setState((prevState: IDiscoverEventsState) => ({
        isLoadingEvents: true,
        hasMoreEvents: false,
        userEventsContainerKey: prevState.userEventsContainerKey + 1,
        isResetFilter: false,
      }));
    }
    ...
    return [
      {
        ...
        content: `${this.localize(
          "mandatoryEventsTab"
        )} ${this.formatEventsCount(this.mandatoryEventsCount)}`,
      },
      {
        key: "all-user-events",
        content: `${this.localize("allEventsTab")} ${this.formatEventsCount(
          this.allEventsCount
        )}`,
      },
    ];
      ...
      <Fabric dir={this.props.dir}>
        <Flex column>
          <Flex space="between" vAlign="center">
            <TabMenu
              defaultTabIndex={MandatoryEventsTabIndex}
              activeTabIndex={this.state.activeTabIndex!}
              tabItems={this.renderTabMenuItems()}
            ...
            <Flex.Item push>
              <Flex gap="gap.medium" vAlign="center">
                ...
                <SearchIcon
                  onClick={() =>
                    this.setState({
                      isMobileSearchBoxOpen: !this.state.isMobileSearchBoxOpen,
                    })
                  }
                />
              </Flex>
            ...
            <Flex className="search-box" vAlign="center" gap="gap.small">
              ...
              <Flex.Item push>
                <CloseIcon
                  ...
                  onClick={this.onMobileSearchBoxClose}
                />
              </Flex.Item>
            </Flex>
          )}
