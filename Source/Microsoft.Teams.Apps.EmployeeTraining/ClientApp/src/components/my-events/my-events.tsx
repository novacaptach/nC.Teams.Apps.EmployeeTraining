...
const RegisteredEventsTabIndex = 0;

/** The tab index for 'Completed events' */
const CompletedEventsTabIndex = 1;
...
class MyEvents extends React.Component<IMyEventsProps, IMyEventsState> {
  ...
  componentDidMount() {
    ...
    const registeredEvents = this.getEventsAsync(
      EventSearchType.RegisteredEventsForUser
    );
    const completedEvents = this.getEventsAsync(
    ...
    this.props.microsoftTeams.tasks.startTask(
      ...
      (error: any, result: any) => {
        if (result && result.isSuccess) {
          const events = this.state.registeredEvents
            ? [...this.state.registeredEvents]
            : [];

          const updatedRegisteredEvents = events.filter(
    ...
    return [
      {
        ...
        content: `${this.localize(
          "registeredEventsTab"
        )} ${this.formatEventsCount(this.registeredEventsCount)}`,
      },
      {
        key: "completed-user-events",
        content: `${this.localize(
          "completedEventsTab"
        )} ${this.formatEventsCount(this.completedEventsCount)}`,
      },
    ];
  };
      ...
      <Fabric dir={this.props.dir}>
        <Flex column>
          <Flex space="between" vAlign="center">
            <TabMenu
              defaultTabIndex={RegisteredEventsTabIndex}
