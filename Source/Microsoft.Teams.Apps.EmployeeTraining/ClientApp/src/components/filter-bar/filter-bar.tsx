...
class FilterBar extends React.Component<IFilterBarProps, IFilterBarState> {
  ...
  constructor(props: IFilterBarProps) {
    ...
    const sortBy: Array<IRadioGroupItem> = getLocalizedSortBy(
      this.localize
    ).map((sortBy: IPostType) => {
      return {
        key: sortBy.id,
        label: sortBy.name,
        value: sortBy.id,
  ...
  componentWillReceiveProps(nextProps: IFilterBarProps) {
    ...
    if (
      ...
      nextProps.categoryList !== this.props.categoryList
    ) {
      this.setState({
    ...
    if (
      nextProps.createdByList &&
      nextProps.createdByList !== this.props.createdByList
    ) {
      this.setState({
        createdByList: nextProps.createdByList.map(
          (value: ITeamsChannelMember) => {
            return {
              isChecked: false,
  ...
  onCategoryStateChange = (categoryValues: Array<ICheckBoxItem>) => {
    ...
    this.setState({
      ...
      categorySelectedCount: selectedCategories ? selectedCategories.length : 0,
    });
  };
  ...
  onCloseIconClick = (event: any) => {
    this.props.onFilterBarCloseClick(
      this.state.categorySelectedCount > 0 ||
        ...
        this.state.selectedSortBy !== SortBy.Recent
    );
    this.isFilterStateChanged = false;
  };
  ...
  onFilterChange = (isFilterItemOpen: boolean) => {
    if (!isFilterItemOpen && this.isFilterStateChanged) {
      const categories = this.state.categoryList
        ? [...this.state.categoryList]
        : [];
      const users = this.state.createdByList
        ? [...this.state.createdByList]
        : [];
      ...
      const filteredCategories = selectedCategories.map(
        (category: ICheckBoxItem) => {
          return category.key;
        }
      );

      const filteredUsers = selectedUsers.map((user: ICheckBoxItem) => {
        return user.key;
      });
              ...
              >
                <div className="filter-bar-item-container">
                  ...
                  <PopupMenuWrapper
                    ...
                    title={
                      this.state.categorySelectedCount > 0
                        ? this.state.categorySelectedCount +
                          " " +
                          this.localize("selected")
                        : this.localize("selectHere")
                    }
                    showSearchBar={true}
                    ...
                    onOpenChange={this.onFilterChange}
                  />
                  <Text
                  ...
                  <PopupMenuWrapper
                    ...
                    title={
                      this.state.createdBySelectedCount > 0
                        ? this.state.createdBySelectedCount +
                          " " +
                          this.localize("selected")
                        : this.localize("selectHere")
                    }
                  ...
                  />
                  <Text
                    content={this.localize("sortBy")}
                    weight="semibold"
                    className="title-text"
                  />
                  <PopupMenuWrapper
                    dir={this.props.dir}
                    title={this.state.sortBy[this.state.selectedSortBy!].name}
                    selectedSortBy={this.state.selectedSortBy!}
                    selectedCount={this.state.createdBySelectedCount}
                    radioGroup={this.state.sortBy}
                    onRadiogroupStateChange={this.onSortByStateChange}
                    onCheckboxStateChange={this.onCreatedByStateChange}
                    onOpenChange={this.onFilterChange}
                  />
                  ...
                  <CloseIcon
                    ...
                    className={
                      this.props.dir === LanguageDirection.Rtl
                        ? "close-icon rtl-left-margin-small"
                        : "close-icon"
                    }
                  />
                </Flex.Item>
                  ...
                  <Flex column gap="gap.small" vAlign="stretch">
                    <Flex className="mobile-filterbar-wrapper">
                      ...
                      <PopupMenuWrapper
                        dir={this.props.dir}
                        title={
                          this.state.sortBy[this.state.selectedSortBy!].name
                        }
                        selectedCount={this.state.createdBySelectedCount}
                        selectedSortBy={this.state.selectedSortBy!}
                        radioGroup={this.state.sortBy}
