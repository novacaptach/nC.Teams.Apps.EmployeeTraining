...
const DeleteDraftEvent: React.FunctionComponent = (props) => {
  ...
  const [isUpdatingStatus, setUpdatingEventStatus] = React.useState(false);
  const [errorGettingEventDetails, setEventDetailsError] = React.useState(
    false
  );
  const [errorUpdatingEventStatus, setUpdateStatusError] = React.useState(
    false
  );
  ...
  const deleteDraftEvent = async () => {
    ...
    const response = await deleteDraftEventAsync(teamId!, eventId!);
        ...
        <Flex>
          <div className="task-module-container close-cancel-event-container">
            <div className="close-cancel-event">
              <Text
                weight="bold"
                content={localize("deleteConfirmationText")}
              />
            </div>
            <Flex
              styles={{
