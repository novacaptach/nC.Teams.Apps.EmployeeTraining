...
import {
  getEventsAsync,
  exportEventDetailsToCSV,
  sendReminder,
} from "../../api/manage-events-api";
...
const DraftEventsTabIndex = 0;
const ActiveEventsTabIndex = 1;
const CompletedEventsTabIndex = 2;
...
> {
    ...
    if (this.props.teamsContext !== nextProps.teamsContext) {
      if (nextProps.teamsContext) {
        ...
        if (
          ...
          const activeEvents = await this.getEventsAsync(
            0,
            EventSearchType.ActiveEventsForTeam
          );
          const draftEvents = await this.getEventsAsync(
            0,
            EventSearchType.DraftEventsForTeam
          );
          const completedEvents = await this.getEventsAsync(
            0,
            EventSearchType.CompletedEventsForTeam
          );

          this.setTotalEventsCount(
            EventSearchType.ActiveEventsForTeam,
            activeEvents.length
          );
          this.setTotalEventsCount(
  ...
  loadEvents = async (pageNumber: number, eventSearchType: EventSearchType) => {
    const events = await this.getEventsAsync(pageNumber, eventSearchType);

    if (events) {
      const loadMoreEvents =
        events.length < Constants.lazyLoadEventsCount ? false : true;
      ...
      switch (eventSearchType) {
        case EventSearchType.DraftEventsForTeam:
          if (pageNumber === 0) {
            ...
            const draftEvents = [...this.state.draftEvents];
            this.setState({
              draftEvents: [...draftEvents, ...events],
              loadMoreEvents,
            });
          }
          break;
        ...
        case EventSearchType.CompletedEventsForTeam:
          if (pageNumber === 0) {
            ...
            const completedEvents = [...this.state.completedEvents];
            this.setState({
              completedEvents: [...completedEvents, ...events],
              loadMoreEvents,
            });
          }
          break;
        ...
        default:
          if (pageNumber === 0) {
            ...
            const activeEvents = [...this.state.activeEvents];
            this.setState({
  ...
  getEvents = (pageNumber: number = 0) => {
    if (pageNumber === 0) {
      this.setState((prevState: IManageEventsState) => ({
        isLoading: true,
        loadMoreEvents: false,
        infiniteScrollKey: prevState.infiniteScrollKey + 1,
      }));
    }

    switch (this.state.activeTabIndex) {
      case DraftEventsTabIndex:
        this.loadEvents(pageNumber, EventSearchType.DraftEventsForTeam).finally(
          () => {
  ...
  formatEventDateAndTime = (eventDetails: IEvent) => {
    if (eventDetails) {
      const eventDayMonthYear = moment
        .utc(eventDetails.startDate)
        .local()
        .format("DD MMM YYYY");
      const eventStartTime = moment
        .utc(eventDetails.startTime)
        .local()
        .format("HH:mm");
      const eventEndTime = moment
        .utc(eventDetails.endTime)
        .local()
        .format("HH:mm");
  ...
  onTabMenuIndexChange = (event: any) => {
    ...
    this.setState({
      isLoading: true,
      searchText: "",
      activeTabIndex: this.state.isMobileView
        ? event?.index!
        : event?.activeIndex!,
    });
    ...
    this.props.microsoftTeams.tasks.startTask(
      {
        ...
        }/create-event?eventId=${eventId}&isDraft=${
          this.state.activeTabIndex === 0 ? true : false
        }`,
      },
      (error: any, result: any) => {
        if (result) {
          if (result.isSuccess) {
            this.setState(
              (prevState: IManageEventsState) => ({
                notification: {
                  id: prevState.notification.id + 1,
                  message: this.localize("updateEventSuccessfulMessage"),
                  type: ActivityStatus.Success,
                },
                activeTabIndex: result.isCreateEvent
                  ? ActiveEventsTabIndex
                  : prevState.activeTabIndex,
              }),
              () => {
  ...
  onSendReminder = async (eventId: string) => {
    const response = await sendReminder(this.teamId, eventId);
      ...
      (error: any, result: any) => {
        if (result) {
          const activeEvents = this.state.activeEvents
            ...
            : [];
          const updatedActiveEvents = activeEvents.filter(
  ...
  onDeleteDraftEvent = (eventId: string, eventName: string) => {
    this.props.microsoftTeams.tasks.startTask(
      ...
      (error: any, result: any) => {
        if (result) {
          const draftEvents = this.state.draftEvents
            ? [...this.state.draftEvents]
            : [];
          const updatedDraftEvents = draftEvents.filter(
            (event: IEvent) => event.eventId !== eventId
          );
  ...
  onExportDetails = async (eventId: string, eventName: string) => {
    const response = await exportEventDetailsToCSV(this.teamId, eventId);
  ...
  getTabMenuItems = () => {
    return [
      {
        key: "draft-events",
        content: `${this.localize("draftEvents")} ${this.formatEventsCount(
          this.draftEventsCount
        )}`,
      },
      {
        key: "active-events",
        content: `${this.localize("activeEvents")} ${this.formatEventsCount(
          this.activeEventsCount
        )}`,
      },
      {
        key: "completed-events",
        content: `${this.localize("completedEvents")} ${this.formatEventsCount(
      ...
      if (result.isSuccess) {
          ...
          (prevState: IManageEventsState) => ({
            notification: {
              ...
              message: result.isDraft
                ? this.localize("createDraftSuccessfulMessage")
                : this.localize("createEventSuccessfulMessage"),
              type: ActivityStatus.Success,
            },
            activeTabIndex: result.isDraft
              ? DraftEventsTabIndex
              : ActiveEventsTabIndex,
          }),
          () => {
        ...
        this.setState((prevState: IManageEventsState) => ({
          notification: {
            id: prevState.notification.id + 1,
            message: result.isDraft
              ? this.localize("createDraftFailureMessage")
              : this.localize("createEventFailureMessage"),
            type: ActivityStatus.Error,
          },
        }));
      }
  ...
  renderEvents() {
    ...
    if (events?.length > 0) {
      if (this.state.isMobileView) {
        const items: Array<any> = events.map((event: IEvent, index: number) => {
          return {
            key: `event-${index}`,
            content: (
                ...
                <Flex space="between">
                  <Flex column>
                    ...
                    <Flex className="list-elements" vAlign="center">
                      ...
                      <Text
                        content={
                          event.type === EventType.InPerson
                            ? event.venue
                            : this.localize("teamsMeeting")
                        }
                        weight="semibold"
                      />
                    </Flex>
                    <Text
                      ...
                      content={this.formatEventDateAndTime(event)}
                    />
                    <Text
                      className="list-elements"
                      content={`${this.localize("noOfRegistrations")}: ${
                        event.registeredAttendeesCount
                      }/${event.maximumNumberOfParticipants}`}
                    />
                  </Flex>
                  <Flex.Item push>
                    {event.status === EventStatus.Cancelled ? (
                      <Button disabled text iconOnly icon={<MoreIcon />} />
                    ) : (
                      <ManageEventsMenu
                        eventDetails={event}
        ...
        const rows: Array<TableRowProps> = events.map((event: IEvent) => {
          const eventVenue = this.getEventType(event);
          const eventDateAndTime: string = this.formatEventDateAndTime(event);
            ...
            items: [
              {
                content: (
                  <Text
                    error={
                      event.status === EventStatus.Cancelled ? true : false
                    }
                    content={
                      event.name +
                      (event.status === EventStatus.Cancelled
                        ? ` (${this.localize("cancelledStatus")})`
                        : "")
                    }
                    weight="bold"
                  />
        ...
        return (
          <Table
            className="manage-events-table"
            header={this.getEventsTableHeader()}
            rows={rows}
          />
        );
      }
    } else {
      return <NoContent message={this.localize("eventsNotAvailable")} />;
    }
  }
  ...
  renderMobileView = () => {
    return (
      <div className="container-manage-events">
        <Flex>
          <Flex.Item push>
            <Flex gap="gap.small">
              <Button
                primary
                content={this.localize("manageCategories")}
                onClick={this.onManageCategoriesClick}
              />
              <Button
                primary
                content={this.localize("createEvent")}
        ...
        <Flex
          className="mobile-menu-manage-events"
          space="between"
          vAlign="center"
        >
          <SplitButton
            ...
            menu={{
              ...
              items: this.getTabMenuItems(),
            }}
            onMenuItemClick={(event: any, data: MenuItemProps | undefined) =>
              this.onTabMenuIndexChange(data)
            }
        ...
        {this.state.isMobileSearchBoxOpen && (
          <Flex
            className="manage-events-mobile-search-box"
            vAlign="center"
            gap="gap.small"
          >
            <Input
        ...
        {this.state.isLoading ? (
          ...
          <div
            key={this.state.infiniteScrollKey}
            className="manage-events-scroll-area-mobile"
          >
            <InfiniteScroll
              pageStart={0}
    ...
    return (
      <div className="container-manage-events">
          ...
          <Flex.Item push={this.props.dir === LanguageDirection.Ltr}>
            <Flex gap="gap.medium" vAlign="center">
              <Input
                ...
                icon={<SearchIcon />}
                iconPosition={
                  this.props.dir === LanguageDirection.Rtl ? "start" : "end"
                }
                placeholder={this.localize("searchForEventsPlaceholder")}
        ...
        {this.state.isLoading ? (
          ...
          <div
            key={this.state.infiniteScrollKey}
            className="manage-events-scroll-area"
          >
            <InfiniteScroll
              pageStart={0}
              loadMore={this.getEvents}
