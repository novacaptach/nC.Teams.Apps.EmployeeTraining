 ...
 */
const ManageEventsMenu: React.FunctionComponent<IManageEventsMenuProps> = (
  props
) => {
  const localize = props.t;

  /** Gets menu items for active events */
  const getActiveEventsMenu: ShorthandCollection<
    MenuItemProps,
    MenuShorthandKinds
  > = [
    {
  ...
  const getDraftEventsMenu: ShorthandCollection<
    MenuItemProps,
    MenuShorthandKinds
  > = [
    {
  ...
  const getCompletedEventsMenu: ShorthandCollection<
    MenuItemProps,
    MenuShorthandKinds
  > = [
    {
  ...
  const getMenuItems = () => {
    switch (props.eventDetails.status) {
      ...
      case EventStatus.Active:
        if (
          new Date() < moment.utc(props.eventDetails.endDate).local().toDate()
        ) {
          ...
          getActiveEventsMenu.unshift({
            ...
            onClick: () =>
              props.onCloseRegistration(props.eventDetails.eventId),
          });
