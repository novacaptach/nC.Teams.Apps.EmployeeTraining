...
class CloseCancelEvent extends React.Component<
  ICloseCancelEventProps,
  ICloseCancelEventState
> {
  ...
  getEventDetailsAsync = async () => {
    ...
    const operationType = queryParam.get("operationType") ?? "0";
    const eventId = queryParam.get("eventId") ?? "0";
    const teamId = queryParam.get("teamId") ?? "0";
    this.isMobileView =
      queryParam.get("isMobileView") === "true" ? true : false;

    const response = await getEventAsync(eventId!, teamId!);

    if (response.status === ResponseStatus.OK && response.data) {
      const eventDetails: IEvent = response.data;
      let eventOperationType: EventOperationType = EventOperationType.None;

      if (
        eventDetails.status === EventStatus.Active &&
        new Date() < moment.utc(eventDetails.endDate).local().toDate()
      ) {
        if (operationType === EventOperationType.CloseRegistration.toString()) {
          ...
          operationType === EventOperationType.CancelEvent.toString()
        ) {
      ...
      this.setState(
        { isLoading: false, eventDetails, eventOperationType },
        () => {
  ...
  onCloseOrCancelEvent = async () => {
    ...
    const teamId = this.state.eventDetails
      ...
      : "0";
    const eventId = this.state.eventDetails
    ...
    if (
      ...
      this.setState({
        isFailedToCloseOrCancelEvent: true,
        isOperationInProgress: false,
      });
    }
  };
