...
const EventCard: React.FunctionComponent<IEventCardProps> = (props) => {
  ...
  const renderFooter = () => {
    const footerElements: Array<JSX.Element> = [];
  ...
  const getAvailableSlots = () => {
    if (
      props.eventDetails.registeredAttendeesCount <
      props.eventDetails.maximumNumberOfParticipants
    ) {
      return (
        props.eventDetails.maximumNumberOfParticipants -
        props.eventDetails.registeredAttendeesCount
      ...
      return (
        <React.Fragment>
          ...
          <Text
            ...
            content={props.eventDetails.venue}
            title={props.eventDetails.venue}
            weight="semibold"
            size="small"
          />
        </React.Fragment>
  ...
  return (
    <Flex column className="event-card" onClick={props.onClick}>
      ...
      {props.eventDetails.selectedColor && (
        <div
          className="event-photo"
          style={{ backgroundColor: props.eventDetails.selectedColor }}
        >
      ...
      <Flex className="event-info" column fill>
        <Flex vAlign="center" gap="gap.medium">
          ...
          >
            <Text
              content={moment
                .utc(props.eventDetails.startDate)
                .local()
                .format("DD")}
              size="large"
              weight="bold"
          ...
          <Flex className="event-details" column vAlign="center" fill>
            <Text
              align={props.dir === "rtl" ? "end" : undefined}
              className={
                props.dir === "rtl"
                  ? "event-category rtl-right-margin-small"
                  : "event-category"
              }
              truncated
            ...
            <Text
              align={props.dir === "rtl" ? "end" : undefined}
              className={props.dir === "rtl" ? "rtl-right-margin-small" : ""}
              truncated
              content={props.eventDetails.name}
              title={props.eventDetails.name}
              weight="bold"
            ...
            <Flex
              className={
                props.dir === "rtl"
                  ? "event-date-and-venue-rtl event-date-and-venue"
        ...
        <Flex className="event-participants" gap="gap.small">
          <Text
            size="small"
            content={`${localize("participantsRegistered")} : ${
              props.eventDetails.registeredAttendeesCount
            }`}
          />
          <Divider vertical />
          <Text
            size="small"
            content={`${localize("availableSlots")} : ${getAvailableSlots()}`}
          />
        </Flex>
        {renderFooter()}
      </Flex>
