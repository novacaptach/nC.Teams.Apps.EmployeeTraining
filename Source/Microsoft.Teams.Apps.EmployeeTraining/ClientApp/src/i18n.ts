...
microsoftTeams.getContext((context: microsoftTeams.Context) => {
  ...
  if (
    supportedCulturesCache.data[SupportedCulturesKey] &&
    defaultCultureCache.data[DefaultCultureKey]
  ) {
    if (
      supportedCulturesCache.data[SupportedCulturesKey].includes(
        context.locale.split("-")[0]
      ) ||
      supportedCulturesCache.data[SupportedCulturesKey].includes(context.locale)
    ) {
    ...
    getSupportedCulturesAsync().then((result: any) => {
      ...
      setDataToCache(
        SUPPORTEDCULTURE_CACHE,
        SupportedCulturesKey,
        supportedCultures
      );
      if (
        supportedCultures.includes(context.locale.split("-")[0]) ||
        supportedCultures.includes(context.locale)
      ) {
        i18n.changeLanguage(context.locale!);
      } else {
