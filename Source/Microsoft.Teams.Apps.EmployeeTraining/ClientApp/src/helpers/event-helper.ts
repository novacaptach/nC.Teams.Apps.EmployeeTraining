...
export const validateSelectedUsers = async (
  selectedUsersAndGroups: Array<ISelectedUserGroup>
) => {
  let users = {
    mandatoryUsers: new Array<string>(),
    optionalUsers: new Array<string>(),
  };
  let groups = {
    mandatoryUsers: new Array<string>(),
    optionalUsers: new Array<string>(),
  ...
  for (let j = 0; j < users.optionalUsers.length; j++) {
    let result = users.mandatoryUsers.find(
      (mandatoryUserId) => mandatoryUserId === users.optionalUsers[j]
    );
    if (!result) {
  ...
  for (let k = 0; k < groups.optionalUsers.length; k++) {
    let result = groups.mandatoryUsers.find(
      (mandatoryUserId) => mandatoryUserId === groups.optionalUsers[k]
    );
    if (!result) {
  ...
  for (let l = 0; l < groups.mandatoryUsers.length; l++) {
    let result = users.optionalUsers.find(
      (optionalUser) => optionalUser === groups.mandatoryUsers[l]
    );
    if (!result) {
  ...
  for (let m = 0; m < groups.optionalUsers.length; m++) {
    let result = users.mandatoryUsers.find(
      (mandatoryUser) => mandatoryUser === groups.optionalUsers[m]
    );
    if (!result) {
  ...
  return {
    mandatoryUsers: filteredUsers.mandatoryUsers,
    optionalUsers: filteredUsers.optionalUsers,
  };
};

export const createEvent = async (
  stepEventState: ICreateEventState,
  teamId: string
) => {
  const eventDetails = { ...stepEventState.eventDetails };
  eventDetails.startDate = moment(eventDetails.startDate)
    .startOf("day")
    .add(eventDetails.startTime?.getHours(), "hours")
    .add(eventDetails.startTime?.getMinutes(), "minutes")
    .utc()
    .toDate();
  eventDetails.endDate = moment(eventDetails.endDate)
    .startOf("day")
    .add(eventDetails.startTime?.getHours(), "hours")
    .add(eventDetails.startTime?.getMinutes(), "minutes")
    .utc()
  ...
  eventDetails.selectedUserOrGroupListJSON = JSON.stringify(
    stepEventState.selectedUserGroups
  );
  const response = await createNewEvent(eventDetails, teamId);
...
export const updateEventDetails = async (
  ...
  const response = await updateEvent(eventDetails, teamId);
...
export const saveEventAsDraftAsync = async (
  stepEventState: ICreateEventState,
  teamId: string
) => {
  const eventDetails = stepEventState.eventDetails;
  ...
  if (stepEventState.isDraft) {
    const response = await updateEventAsDraft(eventDetails, teamId);
    ...
    const response = await saveEventAsDraft(eventDetails, teamId);
 ...
 */
export const formatEventDayAndTimeToShort = (
  startDate: Date,
  startTime: Date,
  endTime: Date
) => {
