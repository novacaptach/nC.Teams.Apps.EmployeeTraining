...
public class EmployeeTrainingActivityMiddleware : IMiddleware
{
    ...
    public EmployeeTrainingActivityMiddleware(
        IOptions<AzureSettings> options,
        ILogger<EmployeeTrainingActivityMiddleware> logger,
        IStringLocalizer<Strings> localizer)
    {
        this.options = options ?? throw new ArgumentNullException(paramName: nameof(options));
        this.logger = logger;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        next = next ?? throw new ArgumentNullException(paramName: nameof(next));
        ...
        if ((turnContext.Activity.Type != ActivityTypes.Event) && !this.IsActivityFromExpectedTenant(turnContext: turnContext))
        {
            this.logger.LogWarning(message: $"Unexpected tenant id {turnContext.Activity.Conversation?.TenantId}");
            if (turnContext.Activity.Type == ActivityTypes.Message)
            {
                await turnContext.SendActivityAsync(textReplyToSend: this.localizer.GetString(name: "InvalidTenantText"), cancellationToken: cancellationToken);
            }

            return;
        }

        await next(cancellationToken: cancellationToken);
    }
