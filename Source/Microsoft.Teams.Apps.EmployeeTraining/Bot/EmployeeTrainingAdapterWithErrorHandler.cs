...
public class EmployeeTrainingAdapterWithErrorHandler : BotFrameworkHttpAdapter
{
        ...
        : base(configuration: configuration)
    {
        ...
        {
            throw new NullReferenceException(message: nameof(EmployeeTrainingActivityMiddleware));
        }

        // Add activity middle ware to the adapter's middle ware pipeline
        this.Use(middleware: employeeTrainingActivityMiddleware);

        this.OnTurnError = async (
            turnContext,
            exception) =>
        {
            ...
            logger.LogError(exception: exception, message: $"Exception caught : {exception.Message}");
            ...
            if (error.Response.StatusCode != HttpStatusCode.TooManyRequests)
            {
                ...
                await turnContext.SendActivityAsync(textReplyToSend: localizer.GetString(name: "ErrorMessage"), cancellationToken: cancellationToken);
            }
            ...
            if (conversationState != null)
            {
                try
                {
                    ...
                    await conversationState.DeleteAsync(turnContext: turnContext);
                }
                catch (Exception ex)
                {
                    logger.LogError(exception: ex, message: $"Exception caught on attempting to Delete ConversationState : {ex.Message}");
                    throw;
                }
            }
        };
